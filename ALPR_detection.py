...
def firebase_upload_image (folder_name1,folder_name2,folder_name3,crop_F,text_F):
    ...
    with open("upload_value.txt", "r") as file:
        content = file.read()
        count = int(content)
        # crop_count=147/9
...
def ALPR_detection(frame_out_name):
    # load model
    yolo_LP_detect = torch.hub.load('yolov5_1', 'custom', path='model/LP_detector_nano_61.pt', force_reload=True, source='local')
    yolo_license_plate = torch.hub.load('yolov5_1', 'custom', path='model/LP_ocr_nano_62.pt', force_reload=True, source='local')
    ...
    camera_url = "rtsp://admin:Dd22864549*@192.168.1.61:554  latency=0 ! rtph264depay ! h264parse ! omxh264dec ! nvvidconv ! videoconvert ! appsink drop=1"
    # camera_url = "rtsp://admin:Dd22864549*@192.168.1.61:554 latency=0 ! rtph264depay ! h264parse ! omxh264dec ! nvvidconv ! video/x-raw, format=BGRx ! appsink drop=1"
    ...
    with open("crop_value.txt", "r") as file:
        content = file.read()
        crop_count = int(content)
    # crop_count=0
    ...
    while(True):
        ...
        if ret:
                ...
                for cc in range(0,2):
                    for ct in range(0,2):
                        ...
                        if lp != "unknown":
                            ...
                            outfile.write(lp)
                            flag = 1
                            #save crop count in txt file
                            with open("crop_value.txt", "w") as file:
                                file.write(str(crop_count))
            ...
            print("No. of current image: ",crop_count)
            # #save crop count in txt file
            # with open("crop_value.txt", "w") as file:
            #     file.write(str(crop_count))
